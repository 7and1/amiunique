...
export const FingerprintSchema = z
  .object({
    ...
    rtc_mdns_obfuscated: z.boolean().optional(),
  ...
  .strip(); // Unknown client fields must not cross the persistence boundary
...
export const DeletionRequestSchema = z
  .object({
    ...
    hash_value: z.string().regex(/^[0-9a-f]{64}$/i, 'Expected a 64-character SHA-256 hash'),
  })
  .strict();

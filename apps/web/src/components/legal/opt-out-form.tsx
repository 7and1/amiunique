...
const hashOptions: { value: HashType; label: string; helper: string }[] = [
  ...
  {
    value: 'software',
    label: 'Silver (Browser hash)',
    helper: 'Same browser profile across sessions',
  },
  {
    value: 'hardware',
    label: 'Gold (Device hash)',
    helper: 'Device-level identifier across browsers',
  },
];
  ...
  return (
    <>
      ...
      <form
        ...
        className="rounded-3xl border border-white/30 bg-white/80 p-8 shadow-card dark:border-white/5 dark:bg-white/5 space-y-6"
      >
        <div>
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            The API stores only this hash while the deletion is pending. It is scrubbed after
            processing; no email address is collected.
          </p>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {hashOptions.map(option => (
              <label
                key={option.value}
                className={`cursor-pointer rounded-2xl border bg-white/70 p-3 text-sm shadow-sm transition hover:border-indigo-300 dark:bg-white/5 ${hashType === option.value ? 'border-indigo-400 dark:border-indigo-500' : 'border-white/30 dark:border-white/10'}`}
              >
                <input
                  type="radio"
                  name="hash-type"
